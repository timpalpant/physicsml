...
from physicsml.models.mace.modules.cg import U_matrix_real
...
class Contraction(torch.nn.Module):
    def __init__(
        ...
        for nu in range(1, correlation + 1):
            ...
            while len(U_matrix.size()) <= nu:
                U_matrix = U_matrix.unsqueeze(0)

            U_matrices[f"{nu}"] = torch.nn.Parameter(U_matrix, requires_grad=False)

        self.U_matrices: torch.nn.ParameterDict = torch.nn.ParameterDict(U_matrices)
        ...
        for i in range(correlation, 0, -1):
            # Shapes defining
            num_params = self.U_mat(i).size()[-1]
            num_equivariance = 2 * irrep_out.lmax + 1
            num_ell = self.U_mat(i).size()[-2]
            ...
            if i == correlation:
                ...
                self.graph_opt_main = opt_einsum_fx.optimize_einsums_full(
                    ...
                    example_inputs=(
                        ...
                        torch.randn((num_elements, num_params, self.num_features)),  # type: ignore
                        torch.randn((BATCH_EXAMPLE, self.num_features, num_ell)),
                        torch.randn((BATCH_EXAMPLE, num_elements)),  # type: ignore
                    ),
                )
        ...
        if not internal_weights:
            self.weights = weights[:-1]  # type: ignore
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        output: torch.Tensor = out.view(out.shape[0], resize_shape)

        return output

    def U_mat(self, nu: int) -> torch.Tensor:
        U_matrix: torch.Tensor = self.U_matrices[f"{nu}"]
        return U_matrix
